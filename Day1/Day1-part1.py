# Your single string with different values separated by line breaks
your_string = """two934seven1
8825eightknfv
sevenoneqbfzntsix55
...
your_strings = your_string.split('\n')
...
# Combine first and last digits from each line into a new number and add to sum

for string in your_strings:
    # Find the first digit
    first_digit = next((char for char in string if char.isdigit()), None)

    # Find the last digit
    last_digit = next((char for char in reversed(string) if char.isdigit()), None)

    # Combine into a new number
    new_number = int(str(first_digit) + str(last_digit) if first_digit and last_digit else "")

    # Add to the sum
    sum_result += new_number

    # Print the results
    # print(f"For string: {string}, First Digit: {first_digit}, Last Digit: {last_digit}, New Number: {new_number}")

# Print the final sum
print(f"Sum of new numbers: {sum_result}")